...
$("#loginButton").click(function () {
    if (checkadmininfo()) {
        $.ajax({
            type: "POST",
            data: {
                'adminindexcode': $("#adminIndexCode").val(),
...
function checkadmininfo() {
    if ($("#adminIndexCode").val() == "") {
        myOpenWindow("请输入管理员账号！")
        return false;
    }
    if ($("#adminPassWord").val() == "") {
        myOpenWindow("请输入管理员密码！")
        return false;
    }
    return true;
}
...
function init(e) {
    ...
    let startY = window.innerHeight / 3.5;
    particles = [];
    targets = [];
